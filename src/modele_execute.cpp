...
int modele_execute(){
    ...
    double zrapport_H2O;
    double delta_angle;	
    // Fout commentée lors du commentaire des sorties car utilisée uniquement pour 
    // tracer l'exécution
    double Fin/*, Fout*/;
    //double Foce, Fbio;
	...
	*exp(2*log((1-excentricite_actuel*0.5*sin(-precession_actuel/180*pi))
	...
	for (int iter=1;iter<=niter;iter++) {
	    ...
	    Fin=Fin0*(1-zalbedo);	
	    // Calcul de la valeur Fout pour le debug traçage de l'exécution
	    // La ligne suivante a été commentée  en même temps que les sorties utilisant Fout	
	...
	double calcul_niveau_mer(double niveau_mer_ancien,double zphig, double zphig_ancien,
	    ...
	    // -> a_mer=140m
	    // SORTIE("zphig="<<zphig);
	    // SORTIE("term2="<<a_mer*(sin((zphig-niveau_calottes_max)*pi/180.)-sin((niveau_calottes_1750-niveau_calottes_max)*pi/180.)));
	    // SORTIE("niveau_mer="<<niveau_mer);
...
//	    }
	...
	double calcul_albedo(double zphig){
		...
		// SORTIE("Aact-B.Ater="<<albedo_actuel-albedo_ter*(niveau_calottes_actuel-phig_crit)/(niveau_calottes_max-phig_crit));
